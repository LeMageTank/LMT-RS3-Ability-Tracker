...
class WizardActionBarMousebindSetupPage(WizardWidget):
    ...
    def initialize_data(self):
        ...
        self._active = True
        print(self._action_bars)

    def screenshot(self):
        return pyautogui.screenshot()

    def load_action_bar_creation_ui(self):
        ...
        self._instruction_label = tkinter.Label(self._instruction_label_frame, text=MOUSEBIND_CREATION_DIRECTIONS.format(self._current_action_bar),
    ...
    def draw_mousebinds(self):
        for widget in self._mousebind_label_widgets:
            widget.destroy()
        self._mousebind_label_widgets = []
        for mousebind in self._mousebind_mappings[self._current_action_bar]:
    ...
    def process_next_action_bar(self):
        self._current_action_bar = self._remaining_action_bars.pop()
        ...
        self._directions_label = tkinter.Label(self._widget,
                                               text="Open your game client with\naction set `{}`\nin view".format(
                                                   self._current_action_bar),
                                               font=text_font)

        self._continue_button = tkinter.Button(self._widget, text="Continue",
                                            font=header_font,
                                            borderwidth=0, highlightthickness=0,
                                            command= lambda: self.load_action_bar_creation_ui())
    ...
    def initialize_mousebind_creation(self):
        self._remaining_action_bars = [action_bar_name for action_bar_name in self._action_bars.keys()]
        self.start_mouse_listener()
        self.process_next_action_bar()
    ...
    def add_mousebind(self, action):
        ...
        mousebind = MousebindAction({'action':action_name, 'x1':x-15, 'y1': y-15,
                    'x2':x+15, 'y2':y+15})
        self._mousebind_mappings[self._current_action_bar].append(mousebind)
